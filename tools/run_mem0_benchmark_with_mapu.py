...
def _require_nonblank_answer(prompt: str) -> str:
    ...
    return (
        ...
        "Before answering, select a compact evidence set: memories or fact_hints "
        "that directly match the exact entity, date, number, preference, status, "
        "or correction asked about. Treat other memories as recall tail; use "
        "them only when they directly answer the question or explicitly correct "
        "a selected memory. Ignore weak, off-topic, or noisy tail evidence.\n"
